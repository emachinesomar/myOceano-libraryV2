...
<div>
  ...
  <div class="group relative">
    ...
    {#if isLeaf && node.path}
      <button
        onclick={handleEdit}
        class="absolute right-1 top-1/2 -translate-y-1/2 p-1 rounded opacity-0 group-hover:opacity-100 text-muted-foreground hover:text-foreground hover:bg-accent transition-all"
  ...
  {#if expanded && !isLeaf}
    <div class="ml-3 pl-3 border-l border-sidebar-border">
      {#each node.children as child (child.name + child.type)}
        <svelte:self node={child} {onMetadataSaved} />
      {/each}
    </div>
  {/if}
</div>
