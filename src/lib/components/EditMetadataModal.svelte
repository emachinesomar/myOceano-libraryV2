<script lang="ts">
  ...
  import { getDocumentMetadata, updateDocumentMetadata } from '$lib/tauri';
  import { toasts } from '$lib/stores/toast';

  let { open = $bindable(false), path = '', onSaved = () => {} }: {
    open: boolean;
    path: string;
    onSaved?: () => void;
  } = $props();
  ...
  async function loadMetadata() {
    ...
    try {
      ...
      // Metadata might not exist yet, use empty values
  ...
  async function handleSave() {
    ...
    try {
      ...
      toasts.error('Error al guardar metadata');
    }
    saving = false;
  }
    ...
    >
      ...
      <div class="flex items-center justify-between px-6 py-4 border-b border-border">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="text-sm font-semibold">Editar Metadata</h2>
            <p class="text-[11px] text-muted-foreground truncate max-w-[300px]">{path.split(/[/\\]/).pop()}</p>
          </div>
        </div>
        <button
          onclick={() => open = false}
